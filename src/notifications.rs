...
/// Minimal port of re_log's `LogMsg` (rerun 0.30.2, MIT OR Apache-2.0):
/// a single log record passed to [`NotificationUi::add_log`].
#[derive(Debug, Clone)]
pub struct LogMsg {
    pub level: Level,
    pub target: String,
...
impl From<log::Level> for NotificationLevel {
    fn from(value: log::Level) -> Self {
        match value {
            log::Level::Trace | log::Level::Debug | log::Level::Info => Self::Info,
...
fn is_relevant(target: &str, level: log::Level) -> bool {
...
impl NotificationUi {
    ...
    /// - If a notifications text contains `"\nDetails:"` the section after that
    ///   will be displayed inside a collapsible details header.
    pub fn add_log(&mut self, message: LogMsg) {
        let LogMsg { level, target, msg } = message;
