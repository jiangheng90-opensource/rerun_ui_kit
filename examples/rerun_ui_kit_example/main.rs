// Run the example with `cargo r -p rerun_ui_kit --example re_ui_example`
...
use rerun_ui_kit::filter_widget::{FilterState, format_matching_text};
use rerun_ui_kit::list_item::ListItemContentButtonsExt as _;
use rerun_ui_kit::menu::menu_style;
use rerun_ui_kit::notifications::NotificationUi;
use rerun_ui_kit::syntax_highlighting::SyntaxHighlightedBuilder;
use rerun_ui_kit::{
...
/// Logger that forwards log records into the notifications UI demo.
/// Replaces re_log::ChannelLogger from the original example.
struct ChannelLogger(crossbeam::channel::Sender<rerun_ui_kit::notifications::LogMsg>);
...
impl log::Log for ChannelLogger {
    ...
    fn log(&self, record: &log::Record<'_>) {
        let _ = self.0.try_send(rerun_ui_kit::notifications::LogMsg {
            level: record.level(),
...
fn main() -> eframe::Result {
    let fullsize_content = rerun_ui_kit::fullsize_content(os::OperatingSystem::default());
    let native_options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_app_id("re_ui_example")
            .with_decorations(!rerun_ui_kit::CUSTOM_WINDOW_DECORATIONS) // Maybe hide the OS-specific "chrome" around the window
            .with_fullsize_content_view(fullsize_content)
            .with_inner_size([1200.0, 800.0])
            .with_title_shown(!fullsize_content)
            .with_titlebar_buttons_shown(!rerun_ui_kit::CUSTOM_WINDOW_DECORATIONS)
            .with_titlebar_shown(!fullsize_content)
            .with_transparent(rerun_ui_kit::CUSTOM_WINDOW_DECORATIONS), // To have rounded corners without decorations we need transparency
    ...
    eframe::run_native(
        ...
        native_options,
        Box::new(move |cc| {
            rerun_ui_kit::apply_style_and_install_loaders(&cc.egui_ctx);
...
pub struct ExampleApp {
    ...
    text_log_rx: Receiver<rerun_ui_kit::notifications::LogMsg>,

    tree: egui_tiles::Tree<Tab>,

    /// regular modal
    modal_handler: rerun_ui_kit::modal::ModalHandler,

    /// modal with full span mode
    full_span_modal_handler: rerun_ui_kit::modal::ModalHandler,
...
impl ExampleApp {
    fn new(ctx: egui::Context) -> Self {
        let (log_tx, text_log_rx) = crossbeam::channel::bounded(256);
        log::set_boxed_logger(Box::new(ChannelLogger(log_tx))).expect("Failed to set logger");
...
impl eframe::App for ExampleApp {
    fn clear_color(&self, _visuals: &egui::Visuals) -> [f32; 4] {
        if rerun_ui_kit::CUSTOM_WINDOW_DECORATIONS {
            [0.0; 4] // transparent
        } else {
    ...
    fn update(&mut self, egui_ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        let left_panel_bottom_section_ui = |ui: &mut egui::Ui| {
            ...
            self.modal_handler.ui(
                ui.ctx(),
                || rerun_ui_kit::modal::ModalWrapper::new("Modal window"),
            ...
            self.full_span_modal_handler.ui(
                ui.ctx(),
                || rerun_ui_kit::modal::ModalWrapper::new("Modal window").full_span_content(true),
                |ui| {
            ...
            ui.section_collapsing_header("Data")
                .with_button(
                    ui.small_icon_button_widget(&rerun_ui_kit::icons::ADD, "Add")
                        .on_menu(|ui| {
                            ui.weak("empty");
                        }),
                )
            ...
            .show_animated(egui_ctx, self.show_left_panel, |ui| {
                ...
                list_item::list_item_scope(ui, "left_panel", |ui| {
                    ...
                    ui.selectable_label_with_icon(
                        ...
                        "foo/bar/baz",
                        false,
                        rerun_ui_kit::LabelStyle::Normal,
                    );
        ...
        if let Some(cmd) = rerun_ui_kit::UICommand::listen_for_kb_shortcut(egui_ctx) {
...
impl ExampleApp {
    fn top_bar(&mut self, _frame: &eframe::Frame, egui_ctx: &egui::Context) {
        ...
        egui::TopBottomPanel::top("top_bar")
            ...
            .show(egui_ctx, |ui| {
                ...
                if !rerun_ui_kit::native_window_bar(egui_ctx.os()) {
    ...
    fn top_bar_ui(&mut self, ui: &mut egui::Ui) {
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ...
            ui.medium_icon_toggle_button(
                &rerun_ui_kit::icons::RIGHT_PANEL_TOGGLE,
                "Selection panel toggle",
                &mut self.show_right_panel,
            );
            ui.medium_icon_toggle_button(
                &rerun_ui_kit::icons::BOTTOM_PANEL_TOGGLE,
                "Time panel toggle",
                &mut self.show_bottom_panel,
            );
            ui.medium_icon_toggle_button(
                &rerun_ui_kit::icons::LEFT_PANEL_TOGGLE,
...
impl egui_tiles::Behavior<Tab> for MyTileTreeBehavior {
    ...
    fn tab_bar_height(&self, style: &egui::Style) -> f32 {
        rerun_ui_kit::design_tokens_of_visuals(&style.visuals).title_bar_height()
    }
